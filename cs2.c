...
/*PK: is this really needed?  Commenting it out:
FOR_ALL_NODES_i
  {
...
i_price = i -> price;
 ...
 a_max = NULL;
 for ( //PK: Find max {p(w) - c(vw): vw outgoing from i, vw open}
    ...
    )
  {
    if ( OPEN ( a )
	 ...
	 ( ( dp = ( ( a -> head ) -> price ) - ( a -> cost ) ) > p_max )
      ...
      {
	if ( i_price < dp ) //PK: if you find an open arc of negative reduced cost, stop
	  {
	    i -> current = a;
	    return ( 1 );
	  }
...
/* finishup */
 ...
 if ( p_max != price_min ) //PK: If there is at least one outgoing open arc
  {
    i -> price   = p_max - epsilon; //PK: Update price of node,
    i -> current = a_max; //PK: and set that arc to be current one for this node.
  }
else
...
// PK: number of nodes, number of *darts*, table of node , table of darts,
//     scale factor?, maximum capacity, table of capacities, output objective value
...

{
  ...
  for (long i = 0; i < num_clients; ++i){
    if (populations[i] > c_max) c_max = populations[i];
  }
